...
class TestgetStartCountStride(unittest.TestCase):
    ...
    def test_fancy(self):
        ...
        try:
            ...
            start, count, stride, put_ind = nc._StartCountStride(
                elem, (3, 4, 5))
        except IndexError:
            pass
    ...
    def test_boolean(self):
        ...
        # Multiple booleans --- The behavior is different from NumPy in this
        # case.
        elem = (np.array([True, True, False]), np.array(
            [True, True, False, True]), slice(None))
        start, count, stride, put_ind = nc._StartCountStride(elem, (3, 4, 5))
        assert_equal(nc._out_array_shape(count), (2, 3, 5))

        try:
            elem = (np.array([True, True, False]), np.array(
                [True, True, True, False]), slice(None))
        except IndexError:
            pass
...
class TestsetStartCountStride(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        start, count, stride, take_ind = nc._StartCountStride(
            elem, (22, 25, 1), [
                'x', 'y', 'time'], grp, (22, 25))
    ...
    def test_booleans(self):
        ...
        elem = (
            [0, 4, 5], np.array([False, True, False, True, True]), slice(None))
        start, count, stride, take_ind = nc._StartCountStride(
            elem, (10, 5, 12), [
                'x', 'y', 'z'], grp, (3, 3, 12))

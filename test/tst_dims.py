...
class DimensionsTestCase(unittest.TestCase):
    ...
    def runTest(self):
        ...
        isunlim = [dimlen is None for dimlen in lens_check]
        ...
        nadd1 = 2
        nadd2 = 4
        v[0:nadd1, :, :, 0:nadd2] = uniform(
            size=(nadd1, LAT_LEN, LON_LEN, nadd2))
        ...
        nadd1 = 8
        nadd2 = 4
        vg[0:nadd1, :, :, 0:nadd2] = uniform(
            size=(nadd1, LAT_LENG, LON_LENG, nadd2))
        lensdict[LEVEL_NAME] = nadd1
        lensdict[TIME_NAME] = nadd2
        for name, dim in g.dimensions.items():
            self.assertTrue(len(dim) == lensdict[name])

...
class VariablesTestCase(unittest.TestCase):
    ...
    def test_issue306(self):
        f = Dataset(self.file, 'w')
        nlats = 7
        lat = f.createDimension('lat', nlats)
        nlons = 12
        lon = f.createDimension('lon', nlons)
        nlevs = 1
        lev = f.createDimension('lev', nlevs)
    ...
    def test_issue300(self):
        ...
        lats = np.arange(nlats)
        lons = np.arange(nlons)

...
datacheck1 = ma.array(
    [0, 5000.0, 4000.0, 0], dtype=np.float, mask=[True, False, False, True])
datacheck2 = ma.array(
    [3000.0, 5000.0, 4000.0, 0], dtype=np.float, mask=[False, False, False, True])
datacheck3 = ma.array(
    [3000.0, 5000.0, 0, 2000.0], dtype=np.float, mask=[False, False, True, False])
...
class PrimitiveTypesTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data = ma.array([1.5678, 99.99, 3.75145, 4.127654], mask=np.array(
            [False, True, False, False], np.bool))
        data.mask[1] = True
        v[:] = data
        f.close()

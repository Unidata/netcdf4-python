...
class VariablesTestCase(unittest.TestCase):
    ...
    def runTest(self):
        ...
        assert(
            chartostring(
                v.units['speed']).item().rstrip().decode(default_encoding) == 'm/s')
        assert(
            chartostring(
                v.units['direction']).item().rstrip().decode(default_encoding) == 'degrees')
        assert(
            chartostring(
                vv.units['speed']).item().rstrip().decode(default_encoding) == 'm/s')
        assert(
            chartostring(
                vv.units['direction']).item().rstrip().decode(default_encoding) == 'degrees')
        f.close()

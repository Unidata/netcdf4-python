...
on compression, set C{zlib=True}.  The C{complevel} keyword regulates the 
...
default value of C{complevel} is 6. Setting C{shuffle=False} will turn off 
the HDF5 shuffle filter, which de-interlaces a block of data before 
compression by reordering the bytes.  The shuffle filter can significantly 
improve compression ratios, and is on by default.  Setting C{fletcher32} 
keyword argument to C{createVariable} to C{True} (it's C{False} by 
...
standard attributes: C{dimensions, ndim, dtype, shape} and
...
names of the dimensions associated with the variable, while the C{ndim}
attribute is the number of variable dimensions. The C{dtype}
...
to improve compression. Ignored if C{zlib=False}. 
...
L{Variable} instance. If C{None}, the data is not truncated. """
    ...
    def filters(self):
        ...
        if self._dset.file_format != 'NETCDF4_CLASSIC':
            ...
            if ierr != NC_NOERR:
                raise RuntimeError(nc_strerror(ierr))
            ierr = nc_inq_var_fletcher32(self._dsetid, self._varid, &ifletcher32)
            if ierr != NC_NOERR:
                raise RuntimeError(nc_strerror(ierr))
            if ideflate:
                filtdict['zlib']=True

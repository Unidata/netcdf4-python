...
def write_netcdf(array,filename,n1dim,n2dim,n3dim,n4dim,zlib=False,complevel=0,shuffle=0,chunking='seq',least_significant_digit=None):
    ...
    foo = file.createVariable('data', 'f8',('n1','n2','n3','n4'),zlib=zlib,complevel=complevel,shuffle=shuffle,chunking=chunking,least_significant_digit=least_significant_digit)
    foo[:] = array
    file.close()

def write_netcdf_unlim(array,filename,n1dim,n2dim,n3dim,n4dim,zlib=False,complevel=0,shuffle=0,chunking='seq',least_significant_digit=None):
    file = netCDF4.Dataset(filename,'w')
    #file.set_fill_off()
...
t1 = time.time()
write_netcdf_unlim(array,'test1.nc',n1dim,n2dim,n3dim,n4dim,chunking=chunking)
print 'writing with no compression took',time.time()-t1,'seconds'
# create a file, put a random array in it.
# use compression.
t1 = time.time()
write_netcdf_unlim(array,'test2.nc',n1dim,n2dim,n3dim,n4dim,zlib=True,complevel=complevel,shuffle=shuffle,chunking=chunking,least_significant_digit=least_significant_digit)

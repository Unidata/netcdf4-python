...
nc = MFnetCDF4.Dataset(files, check=False)
...
the initialization of the MFnetCDF4 instance much slower. Default is 
False.
       """
            ...
            for v in masterRecVar.keys():
                if check:
                    ...
                    if not part.dimensions[vInst.dimensions[0]].isunlimited():
                        raise MFnetCDF4("variable %s is not a record var inside %s" % (v, f))
        ...
        # Attach attributes to the MFnetCDF4.Dataset instance.
...
class _Variable(object):
    ...
    def __getitem__(self, elem):
        ...
        # a possible ellipsis.
        start, count, stride, sliceout = netCDF4._buildStartCountStride(elem, self.shape, self.dimensions, self._dset)
        ...
        for n in range(nv):
            ...
            if idx:
                ...
                # of arrays.
                lstArr.append(netCDF4.Variable.__getitem__(self._recVar[n], tuple(newSlice)))
